...
const LOGO = '/IRIDEMALLORCA_LOGO.png'
...
type TrackCard = { id: string; title: string; fileUrl: string; distanceKm: number; elevationGainM: number; bbox: [number, number, number, number]; svgPath?: string };
type OrderDraft = {
...
const LS_ORDER_DRAFT = 'irm_order_draft_v1';
function loadLS<T>(key: string, fallback: T): T { try { const s = localStorage.getItem(key); return s ? JSON.parse(s) : fallback; } catch { return fallback; } }
...
function RideChat({ room }: { room: string }) {
  ...
  useEffect(() => {
    ...
    const ch: BroadcastChannel = new (window as any).BroadcastChannel('irm_chat');
    busRef.current = ch;
    const handler = (e: MessageEvent) => { const m = e.data as ChatMsg; if (m.room === room) setMessages(x => [...x, m]); };
    ch.addEventListener('message', handler);
    return () => { try { ch.removeEventListener('message', handler); ch.close(); } catch {} busRef.current = null; };
  }, [room]);
      ...
      <CardContent>
        <div className='aspect-[21/9] rounded-xl border bg-white overflow-hidden flex flex-col'>
          ...
          <form onSubmit={(e) => { e.preventDefault(); send(); }} className='border-t p-3 flex gap-2 bg-neutral-100'>
            <input className='flex-1 border rounded-lg px-3 py-2 text-sm' value={text} onChange={(e) => setText(e.target.value)} placeholder='Message…' aria-label='Message' />
            <Button type='submit' className='bg-red-600 hover:bg-red-700 px-4 py-2 text-sm text-white border-0'>Send</Button>
          </form>
        </div>
      </CardContent>
    </Card>
      ...
      <div className='w-full max-w-2xl bg-white rounded-2xl shadow-xl'>
        ...
        <form onSubmit={submitOrder} className='p-4 grid md:grid-cols-2 gap-4'>
          ...
          <div className='md:col-span-2 flex items-center justify-end gap-3 mt-2'>
            <Button type='button' className='border' onClick={onClose}>Cancel</Button>
            <Button type='submit' className='bg-red-600 hover:bg-red-700 text-white border-0'>Send Order</Button>
          </div>
        </form>
      </div>
    </div>
      ...
      <Card className='rounded-2xl'>
        ...
        <CardContent>
          <form onSubmit={add} className='grid md:grid-cols-2 gap-3'>
            ...
            <input name='when' type='datetime-local' step={60} className='w-full border rounded-xl px-3 py-2 md:col-span-2' value={whenLocal} onChange={(e) => setWhenLocal(e.target.value)} required />
            <input name='address' className='border rounded-xl px-3 py-2 md:col-span-2' placeholder='Start address' />
            <textarea name='note' rows={4} className='border rounded-xl px-3 py-2 md:col-span-2' placeholder='Route description (e.g., Calviá loop, Sóller hairpins…)'></textarea>
            ...
            <Button type='submit' className='bg-red-600 hover:bg-red-700 text-white border-0 md:col-span-2'>Post</Button>
          </form>
        </CardContent>
      </Card>
    ...
    <Card className='rounded-2xl'>
      ...
      <CardContent>
        <div className='flex items-center gap-3'>
          <input ref={fileRef} type='file' accept='.gpx' onChange={onUpload} className='border rounded-xl px-3 py-2' />
...
export default function IRideMallorcaPage() {
  ...
  const NAV_ITEMS = [
    ...
    { label: 'Hotels', href: '#hotels', hover: 'hover:bg-yellow-400 hover:text-neutral-900' },
    ...
    <main className='min-h-screen bg-gradient-to-br from-yellow-50 via-white to-red-50 text-neutral-900 selection:bg-yellow-300 selection:text-neutral-900'>
      <header className='sticky top-0 z-40 backdrop-blur bg-white/70 border-b border-neutral-200'>
        <div className='max-w-6xl mx-auto px-4 py-3 flex items-center justify-between'>
          ...
          <nav className='hidden md:flex items-center gap-2 text-sm'>
            {NAV_ITEMS.map((it) => (
              <a key={it.href} href={it.href} className={`px-3 py-2 rounded-md border border-transparent transition-all duration-200 ${it.hover}`}>{it.label}</a>
            ))}
          </nav>
        </div>
      </header>
        ...
        <div className='max-w-6xl mx-auto px-4 py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center'>
          <div>
            ...
            <div className='mt-8 flex gap-3'>
              <Button onClick={() => document.getElementById('tracks')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className='bg-red-600 hover:bg-red-700 text-white border-0 px-5 py-3'>Choose Your Route</Button>
              <Button onClick={() => document.getElementById('jersey')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className='border border-yellow-400 text-yellow-700 hover:bg-yellow-50 px-5 py-3'>Choose Your Jersey</Button>
            </div>
      ...
      <section id='why' className='py-16 md:py-24 bg-white'>
        <div className='max-w-6xl mx-auto px-4 grid md:grid-cols-3 gap-6'>
          {[
            { title: 'Premium Terrain', text: 'Long climbs and excellent road surface.', color: 'bg-red-600 text-white' },
            { title: 'Perfect Climate', text: 'Cyclist-friendly weather from spring to autumn.', color: 'bg-yellow-400 text-neutral-900' },
            { title: 'Ride Culture', text: 'Cafés, shops, and service hubs everywhere.', color: 'bg-red-600 text-white' },
          ].map((f, i) => (
            <Card key={i} className={`rounded-2xl shadow-lg ${f.color}`}>
              <CardHeader><CardTitle className='text-xl font-bold'>{f.title}</CardTitle></CardHeader>
              <CardContent className='text-base'>{f.text}</CardContent>
            </Card>
          ))}
        </div>
      </section>
            ...
            {[
              ...
              <Card key={r.name} className='rounded-2xl overflow-hidden shadow-md'>
                ...
                <CardHeader className='flex flex-row items-center gap-3'>
                  <div className={`p-2 rounded-lg ${i % 2 === 0 ? 'bg-red-50 text-red-600' : 'bg-yellow-50 text-yellow-600'}`}>{r.icon}</div>
      ...
      <section id='jersey' className='py-16 md:py-24 bg-white'>
        <div className='max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-8 items-center'>
          <div className='order-2 md:order-1'>
            ...
            <div className='mt-1 h-1 w-36 bg-gradient-to-r from-red-500 to-yellow-400 rounded'></div>
            <p className='mt-3 text-neutral-600'>Orange–Red–Grey block palette with Spanish flag cuffs. Three rear pockets, YKK zip, quick-dry fabric. Now with <b>UV50+ protection</b>, <b>cooling-touch fabric</b> for hot days, <b>premium materials</b>, and <b>reflective stripes</b> if dusk catches you after sunset.</p>
            <ul className='mt-4 text-neutral-700 list-disc pl-5 space-y-1'>
              <li>Premium polyester/elastane (130–150 g/m²)</li><li>Raw-cut sleeves, silicone hem</li><li>Sizes S–XL</li>
              <li>UV50+ sun protection</li><li>Cooling-touch, heat-optimized fabric</li><li>Reflective safety stripes</li><li>Premium materials & craftsmanship</li>
            </ul>
            <div className='mt-6 flex gap-3'>
              <Button className='bg-neutral-900 hover:bg-neutral-800 text-white border-0' onClick={()=>setOrderOpen(true)}><span>Shop Now</span></Button>
              <Button className='border border-yellow-400 text-yellow-700 hover:bg-yellow-50' onClick={()=>setSizeOpen(true)}>Size Guide</Button>
            </div>
          </div>
      ...
      <section id='chat' className='py-16 md:py-24 bg-white'>
        <div className='max-w-6xl mx-auto px-4'>
          ...
          <p className='text-neutral-600 mb-6'>Join themed rooms. (MVP uses BroadcastChannel — open two tabs to test.)</p>
      ...
      <section id='contact' className='py-16 md:py-24 bg-neutral-100'>
        <div className='max-w-3xl mx-auto px-4 text-center'>
          ...
          <form onSubmit={(e) => e.preventDefault()} className='mt-6 flex flex-col sm:flex-row gap-3 justify-center'>
            ...
            <Button type='submit' className='bg-red-600 hover:bg-red-700 text-white border-0'><Mail className='w-4 h-4 mr-2' />Subscribe</Button>
          </form>
